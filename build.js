...
const compileMainSass = async () => {
	...
	try {
      ...
      - \`${minifiedOutputPath}\``
		...
		log(
			'error',
			`Error compiling \`${mainFile}\`: ${error}`
		)
	}
}
...
const compileModuleSass = async (fileName) => {
	...
	try {
      ...
      - \`${outputPath}\``
		...
		log(
			'error',
			`Error compiling module \`${fileName}\`: ${error}`
		)
	}
}
...
const compileAll = async () => {
	try {
		...
		await Promise.all(
			moduleFiles.map((file) => compileModuleSass(file))
		)
		...
		async (_eventType, filename) => {
			if (filename?.endsWith('.scss')) {
				...
				// Se for um arquivo parcial (_filename.scss), recompila tudo
					...
					log(
						'info',
						'Partial file changed, recompiling all files...'
					)
					await compileAll()
				}
			}
